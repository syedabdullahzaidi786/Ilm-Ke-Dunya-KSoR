---
name: format-checker
description: The record's format rules as a runnable check — frontmatter, filenames, links, structure. Use before handing off any change to knowledge/, when a check fails and you need to fix it, or when unsure whether a document is well-formed. Run with `npm run check` (or node .agents/skills/format-checker/check.mjs).
metadata:
  version: "3.0.0"
---
...
`npm run check` runs `check.mjs`. The rules it holds the record to are the ones in
