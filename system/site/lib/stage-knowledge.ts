 ...
 * may not hang — and this one did, live, on `npm run dev` against a lock whose
...
/**
 * Dev only: carry edits AND ARRIVALS into the stage, so `npm run dev` shows the
 ...
 *
 * Measured before and after, adding a document while `npm run dev` ran:
 ...
 *
 * REMOVALS still wait for the restart `npm run dev` already needs for
 ...
 * next plain `npm run build` stages only public documents — correctly — and the
