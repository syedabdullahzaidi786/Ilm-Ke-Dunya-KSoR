  ...
  if (status === "deprecated" && act(ksor["deprecated"]) === null) {
    throw new Error(
      `${where} is status: deprecated with no ksor.deprecated — a document the record withdrew ` +
        ...
        "Add ksor.deprecated: { by, at } (npm run check refuses this too).",
    );
  }

  // A malformed sources list is a checker finding, not a crash: turning one
  // into an unexplained build failure hides the real message `npm run check` prints.
...
export function predecessorsOf(
  ...
  for (const pointer of pointers) {
    ...
    // page" on the page you are reading. `npm run check` refuses it; this is the
    // second lock.
