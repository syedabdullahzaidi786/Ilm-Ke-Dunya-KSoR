...
function findInstance(start: string): string {
  ...
  throw new Error(
    "instance.md not found — it is the project's identity; build from the project (npm run dev / npm run build at the repo root).",
  );
}
...
function readInstanceName(): string {
  ...
  if (name === null) {
    throw new Error("instance.md carries no name: — it is the project's identity; run npm run check.");
  }
  return name;
}
 ...
 * start, like the identity above — restart `npm run dev` after changing it.
 */
