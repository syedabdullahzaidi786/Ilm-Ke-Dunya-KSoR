 ...
 * nothing checked is a projection nothing governs. `npm run dev` is the review
...
export function readLock(
  ...
  if (!existsSync(file)) {
    refuse(
      ...
      "run `ksor build` first (`npm run build` does), then build the site; `npm run dev` needs no lock",
    );
  }
  let parsed: unknown;
  try {
