...
export async function generateStaticParams() {
  ...
  if (params.length === 0 && folders.length === 0) {
    ...
    // throwing here failed `npm run build` on every freshly scaffolded project
    ...
    // itself is missing.
    throw new Error(
      "the staged record holds neither a document nor a folder index — the stage is missing or empty; run `ksor build` and check knowledge/ (`npm run check` says the same).",
    );
  }
