...
below; a `npm run dev`-only project declares none). Who may read what lives in
`.ksor/governance.yaml`, never here. The body below the frontmatter is the
MCP server's instructions in full; `npm run check` names any other key rather
...
```sh
npm install     # once, after cloning or scaffolding (also fetches the pinned `ksor` tool)
npm run dev         # the site, hot-reloading, at http://localhost:3000
npm run build       # static site into system/site/out/
npm run check       # the format checker — run before handing off any knowledge change
```
...
with honest abstention. It is the climbed rung — not required for `npm run dev`.
...
4. **Bring it up.** Once, then every time:
   ...
   ```sh
   npm run provision  # schema (or migrate) + grant — the privileged acts, run once
   npm run refresh    # build, ingest the record, collect retired generations
   npm run serve      # the MCP server (one supervised process)
   ```
   ...
   **`npm run serve` serves; it does not publish.** It is `ksor serve` and nothing
   else — it opens the port against whatever generation is already active and
   needs no ingest privileges. Publishing is `npm run ingest` (or `npm run refresh`),
   ...
   need write credentials at runtime. So in a deployment, run `npm run provision` and
   `npm run ingest` as DEPLOY steps and run `ksor serve` in the container, where
   ...
   **`npm run refresh` after editing `knowledge/`; `npm run serve` to bring the
   ...
   ```sh
   npx ksor gc --instance instance.md
   ```

   The individual verbs (`npm run schema`, `npm run grant`, `npm run ingest`,
   `npm run gc`) are what `provision` and `refresh` are made of. Reach for them
...
```sh
npm run schema      # apply the DDL (once)
npm run grant       # authorize ingest for this corpus (once)
npm run ingest      # embed knowledge/ into a generation and activate it
npm run serve       # run the MCP server; any other verb: npx ksor <verb>
```
...
previous generation stays as a rollback target; `npx ksor gc` collects
abandoned ones.

### Serving safely (fail-closed posture)

`npm run serve` **refuses to boot unauthenticated** — there is no auth-off
...
```sh
npx ksor takedown --instance instance.md --actor human:you <stable-id> --reason "legal request 2026-08"
npx ksor takedown --instance instance.md --actor human:you <stable-id> --reason "..." --scope subtree
npx ksor takedown --instance instance.md --list      # what is currently denied
npx ksor takedown --instance instance.md --ledger    # who denied what, when
npx ksor takedown --instance instance.md --actor human:you --revoke <entry-id> --reason "..."
```
...
runs over every entry in the ledger at `npm run check`, `ksor build` and ingest, so
...
`npm run build` emits a fully static site (`system/site/out/`) deployable to
...
`npm run build` runs `ksor build` first — every `index.md` regenerated, the
...
`KSOR_BASE_PATH=/repo npm run build` targets sub-path hosting. With audiences
registered in `.ksor/governance.yaml`, plain `npm run build` is the `public`
viewer; `KSOR_AUDIENCE=public,<audience> npm run build` — a comma list, always
...
`.ksor/governance.yaml` authorises it so the first `npm run build` emits a record
...
`0 admitted to a machine surface`, and is working correctly. `npm run dev` shows
...
(`.agents/skills/intake-interview/`). `KSOR_DRAFTS=show npm run build` publishes
...
put the deliberate value on the command, so the owner's ordinary `npm run serve`
...
runtime. So `npm run refresh` is a DEPLOY step you run — from your machine or from
...
- Frontmatter is the KSoR Profile of OKF (the record spec's §2). Required on
  ...
  entry, is not a body change; `npm run check` reads no document history, so it passes an
...
- **`status` is shown only when it is a caveat.** `deprecated` appears as a
  ...
  only under `npm run dev`: a BUILT site has no draft page to label it on.
- `ksor.audience` lists who may read a document; a viewer holds a list that
  ...
  reads as a restriction. `npm run check` refuses any link, `ksor.superseded_by`
...
- **Study attachments.** A document may carry four optional companions named
  ...
  document is missing is refused, by `npm run check` and by `npm run build` alike.
  ...
  **`npm run build` refuses a quiz a reader could pass without reading** (the
  audit runs in the site build, not in `npm run check`), and this is worth knowing
...
- One order drives every surface — the sidebar, `llms.txt`, the home page's
  ...
  next `npm run refresh`, which costs no embedding: only the ordering changed.
...
- `.agents/skills/format-checker/` — the rules above, as a program;
  `npm run check` runs it and its errors explain how to fix themselves.
...
- **Components** — the site is a shadcn/ui project (`system/site/components.json`),
  so `npx shadcn@latest add <name>` writes a component into
