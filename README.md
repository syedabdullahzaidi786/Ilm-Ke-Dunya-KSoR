...
| `/llms.txt` on the DEV server does not change after you approve            | `output: "export"` requires its route handler to be static, so `npm run dev` computes it once per process — the document page beside it updates, that file does not | restart `npm run dev`; the built site is always current, so this is a dev-server artefact only |
...
## Hadith knowledge base

The record includes a complete, auto-generated Sahih al-Bukhari knowledge base
under `knowledge/hadith/sahih-bukhari/`. Each of the 97 books is a separate,
searchable KSoR document with Arabic text and English translation.

### Regenerate Sahih al-Bukhari
...
The script downloads (or loads from cache) the Arabic and English editions,
validates them, and writes one Markdown file per book plus the collection
overview. The raw JSON cache lives in `data/hadith/raw/` and is gitignored —
the script re-downloads it if absent.

### Dataset provenance

| Field | Value |
|-------|-------|
| Dataset | [fawazahmed0/hadith-api](https://github.com/fawazahmed0/hadith-api) |
| API version | 1 |
| Arabic edition | `ara-bukhari` |
| English edition | `eng-bukhari` |
| English translator | Muhsin Khan (as reported by the dataset) |
| License | [The Unlicense](https://unlicense.org) (public domain) |

The Arabic text of Sahih al-Bukhari is a classical 9th-century Islamic text,
well outside any copyright period. The English translation is included in the
dataset under its public-domain release.

### Generated files

| Path | Description |
|------|-------------|
| `knowledge/hadith/overview.md` | Top-level hadith collections overview |
| `knowledge/hadith/sahih-bukhari/overview.md` | Sahih al-Bukhari collection overview |
| `knowledge/hadith/sahih-bukhari/01-revelation.md` … `97-oneness-uniqueness-of-allah-tawheed.md` | One file per book (97 total) |
| `knowledge/hadith/index.md` | Auto-generated by `ksor build` |
| `knowledge/hadith/sahih-bukhari/index.md` | Auto-generated by `ksor build` |

`index.md` files are **always generated by `ksor build`** — never edit them and
never run the import script expecting to produce them. Running `npm run build`
after the import script regenerates them correctly.

### Options

```sh
python scripts/import_bukhari.py --force-download   # ignore cache, re-fetch from CDN
python scripts/import_bukhari.py --dry-run          # validate without writing files
python scripts/import_bukhari.py --help             # all options
```
